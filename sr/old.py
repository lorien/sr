...
Example: sr --python User"""
    ...
    parser.add_option('-a', '--all', default=False, action='store_true',
                      help='Do not ignore hidden files (which name starts with dot)')
    ...
    for path in config.targets:
        ...
        if os.path.isdir(abs_path):
            ...
            for root, dirs, files in os.walk(abs_path):
                ...
                # TODO: dir is bad name, it overrides the ``dir`` builtin function
                for dir in dirs[:]:
                    removed = False
                    ...
                    if not removed:
                        if dir in IGNORE_DIRS:
                            ...
                            if dir in dirs:
                                dirs.remove(dir)
                        # may be should be merged with IGNORE_DIRS?
                        else:
                            for regexp in config.exclude_list:
                                if regexp.search(dir):
                                    ...
                                    # TODO: may be just call break instruction? //lorien
                                    if dir in dirs:
                                        dirs.remove(dir)
